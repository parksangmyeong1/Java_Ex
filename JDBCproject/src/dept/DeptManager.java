...
public class DeptManager {
	...
	void inputData() {
		...
		try {
			...
			Dept dept = new Dept(0, deptData[0], deptData[1]); // 0이어도 상관없다
	...
	// 데이터 수정
	// 사용자로부터 데이터를 받아 수정 -> dao updateDept() 메소드 이용
	void updateData() {
		Connection conn = null;
		...
		try {
			...
			String[] uData = updateData.split(" ");
			
			Dept dept = new Dept(Integer.parseInt(uData[0]), uData[1], uData[2]);
	...
	// 데이터 삭제
	// 사용자로부터 부서번호를 받아 삭제 -> dao deleteDept() 메소드 이용
	void deleteData() {
		Connection conn = null;
