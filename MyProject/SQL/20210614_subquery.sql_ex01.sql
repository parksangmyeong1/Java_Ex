...
where not job = 'ANALYST' and sal < (select distinct sal from emp where job = 'ANALYST');

-- 49. 부하직원이 없는 사원의 이름을 표시하시오.
select ename from emp e where not exists (select * from emp m where e.empno = m.mgr);

-- 50. 부하직원이 있는 사원의 이름을 표시하시오
select ename from emp e where exists (select * from emp m where e.empno = m.mgr);

-- 51. BLAKE와 동일한 부서에 속한 사원의 이름과 입사일을 표시하는 질의를 작성하시오. 
-- ( 단 BLAKE는 제외 )
select ename, hiredate from emp 
where deptno = (select deptno from emp where ename='BLAKE') and not ename='BLAKE';
...
select ename, sal from emp e where exists (select * from emp m where e.mgr = 
