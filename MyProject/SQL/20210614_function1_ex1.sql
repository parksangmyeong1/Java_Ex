...
select hiredate, substr(hiredate, 1, 5) from emp;

--17. SUBSTR 함수를 사용하여 4월에 입사한 사원을 출력하시오.
-- 80/12/17 -> substr(str, 4, 2)
select * --substr(hiredate, 4, 2)
from emp where substr(hiredate, 4, 2) = '04';
...
--    to_char()
select hiredate, to_char(hiredate, 'YY MM MON DY') from emp;
...
select sysdate, to_date('2021/01/01', 'YYYY/MM/DD'),
        sysdate-to_date('2021/01/01', 'YYYY/MM/DD') from dual;
...
select job, max(sal), min(sal), sum(sal), round(avg(sal),0) from emp group by job;
...
select job, min(sal) from emp where mgr is not null 
...
select * from dept; select deptno, decode(deptno,
                        10, 'ACCOUNTING',
                        20, 'RESEARCH',
                        30, 'SALES',
                        40, 'OPERATIONS'
...
round(avg(sal),0), count(*) from emp group by deptno;


-- 조인 이용
select e.deptno, d.loc, d.dname, round(avg(sal),2), count(*)
from emp e, dept d where e.deptno=d.deptno
group by e.deptno, d.loc, d.dname order by e.deptno;
