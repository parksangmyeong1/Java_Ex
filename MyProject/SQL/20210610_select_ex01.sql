...
select ename, sal, sal+300 as upsal from emp;

-- 2. 사원의 이름, 급여, 연간 총 수입을 총 수입이 많은 것부터 작은 순으로 출력하시오, 연간 총수입은 월급에 12를 곱한 후 $100의 상여금을 더해서 계산하시오.
select ename, sal, sal*12+100 as yearSal from emp order by yearSal desc;
...
select ename, sal from emp where sal not between 2000 and 3000 order by sal; 
...
select ename,hiredate from emp where hiredate like '81/%/%' order by hiredate;
...
select ename, job, sal from emp where job in('CLERK','SALESMAN') and sal not in(950,1600,1300) order by sal;
