...
--on e.deptno=d.deptno
using (deptno) where e.ename='SCOTT';
...
where sal between s.losal and s.hisal order by e.ename;

-- 회원별 구매 내역출력
select c.name--, count(o.orderid) as "구매횟수", avg(saleprice)
from orders o , customer c where o.custid(+)=c.custid group by c.name order by c.name;
